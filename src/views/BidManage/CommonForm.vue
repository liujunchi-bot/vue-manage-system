...
<el-form :inline="inline" :model="form" ref="form" label-width="100px">
  <el-form-item v-for="item in formLabel" :key="item.model" :label="item.label">
    ...
    <el-input v-model="form[item.model]" :placeholder="'自动编号' + item.label" v-if="item.label === code" disabled></el-input>
    ...
    <el-date-picker v-model="form[item.model]" type="date" placeholder="选择日期" v-if="item.type === 'date'" value-format="yyyy-MM-dd">
    </el-date-picker>
  </el-form-item>
  <el-form-item>
    <slot></slot>
